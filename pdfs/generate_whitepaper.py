...
def generate_whitepaper():
    ...
    pdf.sub_heading('Aviation & Aerospace Supply Chain Intelligence')
    pdf.tech_badge('Aircraft Order Book & Backlog Intelligence')
    pdf.tech_badge('Engine MRO Cycle Revenue Predictor')
    pdf.tech_badge('Aviation Fuel Hedging Optimiser')
    pdf.tech_badge('Airline Fleet Renewal Demand Model')
    pdf.tech_badge('Aerospace Supply Chain Bottleneck Tracker')
    pdf.tech_badge('Aircraft Residual Value Predictor')
    pdf.tech_badge('Air Traffic Demand Nowcaster')
    pdf.tech_badge('Defence vs Commercial Mix Optimiser')
    pdf.tech_badge('Narrowbody vs Widebody Demand Cycle Model')
    pdf.ln(10)

    pdf.sub_heading('Global Automotive Manufacturing Intelligence')
    pdf.tech_badge('Global Vehicle Production Nowcaster')
    pdf.tech_badge('EV Transition Cannibalisation Model')
    pdf.tech_badge('Battery Cost Learning Curve Predictor')
    pdf.tech_badge('Automotive Chip Allocation Intelligence')
    pdf.tech_badge('Dealership Inventory & Incentive Tracker')
    pdf.tech_badge('Autonomous Driving Regulatory Timeline Model')
    pdf.tech_badge('ICE-to-EV Margin Crossover Predictor')
    pdf.tech_badge('Supplier Tier Dependency Graph (Automotive)')
    pdf.tech_badge('Used Vehicle Price Elasticity Engine')
    pdf.ln(10)

    pdf.sub_heading('Precision Agriculture & Food Security Intelligence')
    pdf.tech_badge('Precision Agriculture Yield Forecaster')
    pdf.tech_badge('Crop Commodity Price-to-Equipment Demand Model')
    pdf.tech_badge('Autonomous Farm Fleet ROI Calculator')
    pdf.tech_badge('Soil & Weather-Adjusted Planting Intelligence')
    pdf.tech_badge('Global Food Security Stress Index')
    pdf.tech_badge('Fertiliser & Input Cost Predictor')
    pdf.tech_badge('Land Value & Farmland Investment Model')
    pdf.tech_badge('Water Rights & Irrigation Scarcity Scorer')
    pdf.tech_badge('Ag-Tech Adoption Curve Intelligence')
    pdf.ln(10)

    pdf.sub_heading('Maritime & Global Trade Flow Intelligence')
    pdf.tech_badge('Global Container Freight Rate Predictor')
    pdf.tech_badge('Port Congestion & Dwell Time Nowcaster')
    pdf.tech_badge('Trade Route Disruption Probability Engine')
    pdf.tech_badge('Vessel Tracking & Dark Fleet Intelligence')
    pdf.tech_badge('Bill of Lading & Customs Flow Analytics')
    pdf.tech_badge('Shipping Fuel & Emissions Cost Model')
    pdf.tech_badge('Dry Bulk vs Tanker vs Container Cycle Detector')
    pdf.tech_badge('Suez/Panama Canal Chokepoint Risk Scorer')
    pdf.tech_badge('Global Inventory-in-Transit Tracker')
    pdf.ln(10)

    pdf.sub_heading('Exchange Infrastructure & Derivatives Intelligence')
    pdf.tech_badge('Open Interest & Volume Anomaly Detector')
    pdf.tech_badge('Futures Curve Term Structure Intelligence')
    pdf.tech_badge('Options Skew & Volatility Surface Modeller')
    pdf.tech_badge('Cross-Exchange Basis Trade Arbitrage')
    pdf.tech_badge('Clearing House Margin Stress Predictor')
    pdf.tech_badge('Commodity Delivery & Squeeze Risk Engine')
    pdf.tech_badge('VIX Term Structure Regime Classifier')
    pdf.tech_badge('ETF Creation/Redemption Flow Intelligence')
    pdf.tech_badge('Exchange Microstructure Latency Mapper')
    pdf.ln(10)

    pdf.sub_heading('Ratings & Credit Intelligence Supremacy')
    pdf.tech_badge('Sovereign Credit Rating Predictor')
    pdf.tech_badge('Corporate Default Probability Engine')
    pdf.tech_badge('Rating Migration & Fallen Angel Detector')
    pdf.tech_badge('ESG Score Divergence Arbitrage')
    pdf.tech_badge('Index Inclusion/Exclusion Predictor')
    pdf.tech_badge('Credit Spread Term Structure Modeller')
    pdf.tech_badge('Structured Product Rating Decomposer')
    pdf.tech_badge('Data Vendor Lock-In Escape Intelligence')
    pdf.tech_badge('Alternative Data Quality Scoring Engine')
    pdf.ln(10)

    pdf.sub_heading('Catastrophe & Reinsurance Intelligence')
    pdf.tech_badge('Catastrophe Bond Pricing Engine')
    pdf.tech_badge('Natural Disaster Probability Modeller')
    pdf.tech_badge('Insurance-Linked Securities Alpha')
    pdf.tech_badge('Climate Change Loss Projection Model')
    pdf.tech_badge('Pandemic & Epidemic Risk Quantifier')
    pdf.tech_badge('Reinsurance Tower Optimisation Engine')
    pdf.tech_badge('Mortality & Longevity Risk Modeller')
    pdf.tech_badge('Cyber Risk Accumulation Tracker')
    pdf.tech_badge('Loss Reserve Adequacy Predictor')
    pdf.ln(10)

    pdf.sub_heading('Chip Architecture & Edge Compute Intelligence')
    pdf.tech_badge('ARM License Revenue Forecaster')
    pdf.tech_badge('Edge AI Chip Design Win Tracker')
    pdf.tech_badge('Instruction Set Architecture Market Model')
    pdf.tech_badge('RISC-V vs ARM Competitive Threat Scorer')
    pdf.tech_badge('IoT & Embedded Compute TAM Forecaster')
    pdf.tech_badge('Mobile SoC Power-Performance Benchmarking')
    pdf.tech_badge('Arm-based Server Adoption Curve Model')
    pdf.tech_badge('Custom Silicon Royalty Rate Intelligence')
    pdf.tech_badge('Chip IP Licensing Moat Quantifier')
    pdf.ln(10)

    pdf.sub_heading('Venture & Tech Cycle Intelligence')
    pdf.tech_badge('Late-Stage Venture Valuation Model')
    pdf.tech_badge('Tech IPO Window Predictor')
    pdf.tech_badge('Vision Fund Portfolio Contagion Analyser')
    pdf.tech_badge('Startup Burn Rate & Runway Calculator')
    pdf.tech_badge('Unicorn Markdown Probability Scorer')
    pdf.tech_badge('VC Fund Vintage Year Performance Predictor')
    pdf.tech_badge('AI Hype Cycle Position Tracker')
    pdf.tech_badge('SPACmaggedon & De-SPAC Risk Scorer')
    pdf.tech_badge('Secondary Market Private Equity Pricing')
    pdf.ln(10)

    pdf.sub_heading('Enterprise Data Gravity Intelligence')
    pdf.tech_badge('ERP Migration & Cloud Transition Revenue Model')
    pdf.tech_badge('Enterprise Software Seat Expansion Predictor')
    pdf.tech_badge('Database Market Share Shift Tracker')
    pdf.tech_badge('Legacy System Technical Debt Quantifier')
    pdf.tech_badge('SaaS Net Revenue Retention Forecaster')
    pdf.tech_badge('On-Premise to Cloud Spend Migration Curve')
    pdf.tech_badge('Enterprise AI Adoption Readiness Scorer')
    pdf.tech_badge('Vendor Consolidation Wave Predictor')
    pdf.tech_badge('Total Cost of Ownership Comparison Engine')
    pdf.ln(10)

    pdf.sub_heading('SaaS & Enterprise Workflow Intelligence')
    pdf.tech_badge('CRM Pipeline-to-Revenue Conversion Predictor')
    pdf.tech_badge('Customer Churn Propensity Engine')
    pdf.tech_badge('SaaS Cohort LTV/CAC Optimiser')
    pdf.tech_badge('IT Service Desk Ticket Volume Forecaster')
    pdf.tech_badge('Workflow Automation ROI Calculator')
    pdf.tech_badge('Platform Ecosystem Stickiness Scorer')
    pdf.tech_badge('Enterprise Contract Renewal Predictor')
    pdf.tech_badge('App Marketplace Revenue Share Model')
    pdf.tech_badge('Multi-Cloud Workflow Orchestration Intelligence')
    pdf.ln(10)

    pdf.sub_heading('Streaming & Attention Economy Intelligence')
    pdf.tech_badge('Subscriber Growth & Churn Predictor (Streaming)')
    pdf.tech_badge('Content ROI & Greenlight Prediction Engine')
    pdf.tech_badge('Attention Economy Share-of-Eyeball Tracker')
    pdf.tech_badge('Ad-Supported vs Premium Tier Migration Model')
    pdf.tech_badge('IP Franchise Valuation Engine')
    pdf.tech_badge('Sports Rights Bidding Intelligence')
    pdf.tech_badge('Content Licensing & Library Depreciation Model')
    pdf.tech_badge('Streaming Wars Market Share Nowcaster')
    pdf.tech_badge('Cultural Trend & Viral Content Predictor')
    pdf.ln(10)

    pdf.sub_heading('Gig Economy & Travel Demand Intelligence')
    pdf.tech_badge('Ride-Hail & Delivery Demand Nowcaster')
    pdf.tech_badge('Dynamic Surge Pricing Optimiser')
    pdf.tech_badge('Gig Worker Supply & Wage Elasticity Model')
    pdf.tech_badge('Travel Demand Recovery & Seasonality Engine')
    pdf.tech_badge('Short-Term Rental Yield Optimiser')
    pdf.tech_badge('Autonomous Vehicle Disruption Timeline Model')
    pdf.tech_badge('Urban Mobility Modal Shift Predictor')
    pdf.tech_badge('Experience Economy Pricing Intelligence')
    pdf.tech_badge('Regulatory Gig Economy Classification Tracker')
    pdf.ln(10)

    pdf.sub_heading('Industrial Automation & Digital Twin Intelligence')
    pdf.tech_badge('Digital Twin Asset Performance Predictor')
    pdf.tech_badge('Industrial IoT Sensor Anomaly Detector')
    pdf.tech_badge('Factory Automation ROI & Payback Model')
    pdf.tech_badge('Predictive Maintenance Revenue Forecaster')
    pdf.tech_badge('Smart Grid Load Balancing Intelligence')
    pdf.tech_badge('Building Energy Management Optimiser')
    pdf.tech_badge('Industrial Supply Chain Digital Thread')
    pdf.tech_badge('Robotics Adoption Curve by Industry')
    pdf.tech_badge('Carbon Footprint per Unit of Production Tracker')
    pdf.ln(10)

    pdf.sub_heading('India Digital Economy Intelligence')
    pdf.tech_badge('Jio Platform Subscriber Economics Model')
    pdf.tech_badge('India Digital Payment Velocity Tracker')
    pdf.tech_badge('Indian Retail & E-Commerce Penetration Curve')
    pdf.tech_badge('Rupee & Indian Bond Market Sensitivity Engine')
    pdf.tech_badge('India Energy Transition & Refining Margin Model')
    pdf.tech_badge('Bollywood & Cricket Media Rights Valuation')
    pdf.tech_badge('India Demographic Dividend Economic Model')
    pdf.tech_badge('Reliance Conglomerate Cross-Subsidy Analyser')
    pdf.tech_badge('India Regulatory & Policy Shift Predictor')
    pdf.ln(10)

    pdf.sub_heading('European Energy Transition Intelligence')
    pdf.tech_badge('European Carbon Credit & ETS Price Model')
    pdf.tech_badge('LNG Spot vs Contract Arbitrage Engine')
    pdf.tech_badge('Refinery Crack Spread Optimiser')
    pdf.tech_badge('Renewable Energy Cannibalisation Curve')
    pdf.tech_badge('Hydrogen Economy Timeline & Cost Predictor')
    pdf.tech_badge('Oil Major Transition Risk Scorer')
    pdf.tech_badge('European Gas Storage & Winter Stress Model')
    pdf.tech_badge('Offshore Wind Capacity Factor Intelligence')
    pdf.tech_badge('Energy Company Stranded Asset Quantifier')
    pdf.ln(10)

    pdf.sub_heading('Central Bank Omniscience')
    pdf.tech_badge('Fed Dot Plot & Rate Path Predictor')
    pdf.tech_badge('ECB Forward Guidance Decoder')
    pdf.tech_badge('BOJ Yield Curve Control Break Probability')
    pdf.tech_badge('PBOC Liquidity Injection Nowcaster')
    pdf.tech_badge('Central Bank Balance Sheet Trajectory Model')
    pdf.tech_badge('Inflation Expectations Decomposer')
    pdf.tech_badge('Currency Intervention Detection Engine')
    pdf.tech_badge('Quantitative Tightening Impact Simulator')
    pdf.tech_badge('Emerging Market Central Bank Contagion Model')
    pdf.ln(10)

    pdf.sub_heading('Climate & Weather Alpha')
    pdf.tech_badge('Weather Derivative Pricing Engine')
    pdf.tech_badge('Crop Yield Satellite Forecaster')
    pdf.tech_badge('Hurricane & Typhoon Path Financial Impact Model')
    pdf.tech_badge('Drought & Flood Agricultural Loss Predictor')
    pdf.tech_badge('Carbon Offset & Credit Verification Intelligence')
    pdf.tech_badge('Sea Level Rise Real Estate Impact Model')
    pdf.tech_badge('Wildfire Risk & Insurance Pricing Engine')
    pdf.tech_badge('El Nino / La Nina Commodity Impact Predictor')
    pdf.tech_badge('Climate Migration Economic Impact Model')
    pdf.ln(10)

    pdf.sub_heading('Quantum Computing Readiness')
    pdf.tech_badge('Post-Quantum Cryptography Migration Planner')
    pdf.tech_badge('Quantum Advantage Timeline Forecaster')
    pdf.tech_badge('Quantum-Resistant Portfolio Encryption')
    pdf.tech_badge('Quantum Monte Carlo Acceleration Engine')
    pdf.tech_badge('Quantum Key Distribution Readiness Scorer')
    pdf.tech_badge('Quantum Machine Learning Feature Discovery')
    pdf.tech_badge("Shor's Algorithm Threat Assessment (RSA/ECC)")
    pdf.tech_badge('Quantum Hardware Vendor Benchmark Tracker')
    pdf.tech_badge('Hybrid Classical-Quantum Optimisation Pipeline')
    pdf.ln(10)

    pdf.sub_heading('Geopolitical War Gaming Engine')
    pdf.tech_badge('Conflict Escalation Probability Engine')
    pdf.tech_badge('Sanctions Impact Cascade Simulator')
    pdf.tech_badge('Regime Change & Election Outcome Predictor')
    pdf.tech_badge('Trade War Tariff Scenario Modeller')
    pdf.tech_badge('Refugee & Migration Flow Economic Impact')
    pdf.tech_badge('Nuclear Proliferation Risk Scorer')
    pdf.tech_badge('Alliance & Treaty Stability Monitor')
    pdf.tech_badge('Cyber Warfare Attribution & Market Impact')
    pdf.tech_badge('Failed State Contagion & Spillover Model')
    pdf.ln(10)
